...
import { UIAdaptation, LLMConfig, UIComponent, LLMProvider } from "./types";
import { adaptlyLogger } from "./logger";

// Enhanced LLM Service class for managing AI interactions with multiple providers
export class EnhancedLLMService {
  ...
  private async initializeModel() {
    try {
      adaptlyLogger.info(
        `Initializing ${this.provider} LLM with API key:`,
        this.config.apiKey ? "***" + this.config.apiKey.slice(-4) : "MISSING"
      );

      // Set the API key in the environment for the respective AI SDK
      if (this.config.apiKey) {
        switch (this.provider) {
          case "google":
            process.env.GOOGLE_GENERATIVE_AI_API_KEY = this.config.apiKey;
            this.model = google(this.config.model);
            break;
          case "openai":
      ...
      adaptlyLogger.info(
        `Enhanced LLM Service initialized with ${this.provider}:`,
        this.config.model
      );
    } catch (error) {
      adaptlyLogger.error(
        `Failed to initialize ${this.provider} LLM service:`,
        error
      );
  ...
  async processUserRequest(
    ...
    if (!this.isInitialized) {
      return {
        success: false,
        error: `${this.provider} LLM service not initialized. Please check your API key.`,
      };
    }
    ...
    try {
      ...
      const result = await generateText({
        model: this.model as unknown as string,
...
- User says "I want something different" → Use first available component with "Describe your vision"`,
      ...
      adaptlyLogger.debug(`${this.provider} LLM Response:`, result.text);
      ...
      try {
        ...
        if (responseMatch) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              `${this.provider} LLM parsed response:`,
              parsedResponse
            );
            console.log(
              `${this.provider} LLM reasoning field:`,
              parsedResponse.reasoning
            );
            console.log(
              `${this.provider} LLM reasoning type:`,
              typeof parsedResponse.reasoning
            );
            console.log(
              `${this.provider} LLM components:`,
        ...
        adaptlyLogger.warn(
          `Failed to parse ${this.provider} LLM JSON response:`,
          parseError
        );
      }
      ...
      adaptlyLogger.error(
        `Error processing user request with ${this.provider}:`,
        error
      );
      return {
        success: false,
        error: `Failed to process request with ${this.provider}: ${
  ...
  async getComponentSuggestions() {
    if (!this.isInitialized) {
      return {
        success: false,
        error: `${this.provider} LLM service not initialized`,
      };
    }
  }
...
Provide suggestions for improvement.`,
      ...
      return {
        success: false,
        error: `Failed to analyze layout with ${this.provider}: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
      };
    }
